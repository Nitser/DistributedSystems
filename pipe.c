...
int send(void * self, local_id dst, const Message * msg) {
	...
	ssize_t write_size = write(w_fd, (const void*)msg, message_size);
	printf("Can`t write to pipe; message_size = %zd; write_size = %zd\n", message_size, write_size);
...
int receive_any(void * self, Message * msg) {
	...
	for (local_id id = 0; id < pipes->quantity; id++) {
		...
		if (r_fd != -1) {
			...
			printf("self.id = %d; message: %s\n", pipes->id, msg->s_payload);
		}
	}
	return 0;
}
